...
export function calculateAbsoluteEpisode(
  ...
  if (Array.isArray(epsData)) {
    ...
    console.log(
      `DEBUG: Array format. Seasons before current (${season}):`,
      JSON.stringify(seasonsBefore)
    );
  ...
  console.log(
    `DEBUG: Fixed format. Calculation: (${season} - 1) * ${epsData} + ${episode} = ${result}`
  );
...
export function getSeasonAndEpisode(
  ...
  console.log(
    `DEBUG: getSeasonAndEpisode for absolute episode ${absoluteEpisode} of "${showName}"`
  );
  ...
  if (Array.isArray(epsData)) {
    ...
    while (season <= epsData.length) {
      ...
      console.log(
        `DEBUG: Season ${season} has ${episodesInSeason} episodes, remaining: ${remainingEpisodes}`
      );
    ...
    const calculatedSeason =
      epsData.length + additionalSeasons + (remainingEpisodeInLastSeason > 0 ? 1 : 0);
    const calculatedEpisode =
      remainingEpisodeInLastSeason === 0 ? lastSeasonEpisodeCount : remainingEpisodeInLastSeason;

    console.log(
      `DEBUG: Episode ${absoluteEpisode} is beyond defined seasons. Calculated S${calculatedSeason}E${calculatedEpisode}`
    );
    ...
    console.log(
      `DEBUG: Fixed format (${epsData} per season). Episode ${absoluteEpisode} is S${season}E${episode}`
    );
