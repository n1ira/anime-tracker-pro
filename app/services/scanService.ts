...
export async function getScanState() {
  try {
    ...
    if (scanState[0].currentShowId) {
      try {
        ...
        if (show && show.length > 0) {
          currentShow = show[0];
          await logInfo(
            `Retrieved currentShow for scan state: ${currentShow.title} (ID: ${currentShow.id})`
          );
        } else {
    ...
    const formattedScanState = {
      ...
      updatedAt: scanState[0].updatedAt
        ? new Date(scanState[0].updatedAt).toISOString()
    ...
    await logInfo(
      `Returning scan state: isScanning=${formattedScanState.isScanning}, status=${formattedScanState.status}, currentShowId=${formattedScanState.currentShowId}, hasCurrentShow=${currentShow !== null}`
    );
...
export async function updateScanState(
  ...
  try {
    ...
    await logInfo(
      `Updated scan state: isScanning=${isScanning}, status=${truncatedStatus}, currentShowId=${currentShowId}`
    );
  } catch (error) {
...
export async function scanShow(showId: number, origin: string, isPartOfBatchScan: boolean = false) {
  try {
    ...
    if (downloadedEpisodes.length === 0) {
      ...
      await logInfo(
        `Highest downloaded episode for ${show[0].title} is S${highestDownloaded.season}E${highestDownloaded.episodeNumber}, looking for S${nextSeason}E${nextEpisode}`
      );
    }
    ...
    await logInfo(
      `Looking for ${show[0].title} S${nextEpisodeToDownload.season}E${nextEpisodeToDownload.episode}`
    );
    ...
    if (!searchResult || !searchResult.success) {
      await logInfo(
        `No matches found for ${show[0].title} S${nextEpisodeToDownload.season}E${nextEpisodeToDownload.episode}`
      );
    ...
    const existingEpisode = allEpisodes.find(
      ep => ep.episodeNumber === nextEpisodeToDownload.episode
    );
    ...
    if (existingEpisode) {
      ...
      await logInfo(
        `Updated episode ${show[0].title} S${nextEpisodeToDownload.season}E${nextEpisodeToDownload.episode}`
      );
    } else {
      // Create new episode
      ...
      await logInfo(
        `Created new episode ${show[0].title} S${nextEpisodeToDownload.season}E${nextEpisodeToDownload.episode}`
      );
    }
