...
export async function searchTorrents(showId: number, season: number, episode: number) {
  try {
    ...
    // Log the search
    await logInfo(`Searching for ${show[0].title} S${season}E${episode} (Absolute Episode ${absoluteEpisode})`);
    ...
    if (parsedResults.length === 0) {
      await logInfo(`No valid episodes found for ${show[0].title} S${season}E${episode} after parsing`);
      return { success: false, message: "No matches found" };
    }
