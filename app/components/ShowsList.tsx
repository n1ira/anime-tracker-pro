  ...
  const renderEpisodesCount = (showId: number) => {
    ...
    return (
      <div className="text-sm text-muted-foreground">
        <span>Total: {getCurrentEpisode(showId)}/{getTotalEpisodes(showId)}</span>
              ...
              {shows.map((show) => (
                ...
                >
                  <div className={`flex ${hasMultipleSeasons(show.id) ? 'flex-col w-full' : 'items-center justify-between w-full'}`}>
                    ...
                    <div className={`flex items-center ${hasMultipleSeasons(show.id) ? 'mt-2 w-full justify-between' : 'space-x-2'}`}>
      ...
      <AlertDialog open={!!showIdToDelete} onOpenChange={(open) => {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the show &quot;{shows.find(show => show.id === showIdToDelete)?.title}&quot; and all its episodes.
