...
export function summarizeLogs(logs: any[]) {
  ...
  let specificShowOperations = new Map<string, any>(); // Track operations by show name
  
  // Look for evidence of scan completion
  const hasScanCompleted = sortedLogs.some(log => 
  ...
  for (const log of sortedLogs) {
    ...
    // Handle found episode and download success
            ...
            (log.message.includes('Found episode') || log.message.includes('Marked') && log.message.includes('as downloaded'))) {
    ...
    else if ((log.message.includes('stopping scan') || 
             log.message.includes('No results found') || 
             log.message.includes('No valid episodes found')) && currentOperation) {
  ...
  if (specificShowOperations.size > 0 && scanOperations.filter(op => op.show !== 'All Shows').length === 0) {
    specificShowOperations.forEach(operation => {
      scanOperations.push({...operation});
    });
  }
  ...
  const result = uniqueSummaries.sort((a, b) => 
  ...
  if (result.length === 0 && sortedLogs.length > 0 && hasScanCompleted) {
    ...
    if (lastScanCompletionLog) {
      ...
      const showName = match && match[1] && match[1] !== 'all shows' 
        ? match[1].trim() 
        : 'All Shows';
