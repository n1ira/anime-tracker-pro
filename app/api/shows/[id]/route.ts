...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const rangeChanged =
      ...
      (data.episodesPerSeason !== undefined &&
        data.episodesPerSeason !== currentShow[0].episodesPerSeason);
...
async function regenerateEpisodes(showId: number, show: any) {
  try {
    console.log(
      `Regenerating episodes for show #${showId} with range: S${show.startSeason || 1}E${show.startEpisode || 1} to S${show.endSeason || 1}E${show.endEpisode || 1}`
    );
    ...
    await db.delete(episodesTable).where(eq(episodesTable.showId, showId));
    ...
    for (let season = startSeason; season <= endSeason; season++) {
      const seasonEpisodeCount = Array.isArray(episodesPerSeason)
        ? season <= episodesPerSeason.length
          ? episodesPerSeason[season - 1]
          : episodesPerSeason[episodesPerSeason.length - 1]
        : episodesPerSeason;
      ...
      for (let episodeInSeason = firstEpisode; episodeInSeason <= lastEpisode; episodeInSeason++) {
        ...
        console.log(
          `Created episode S${season}E${episodeInSeason} with absolute number ${absoluteEpisode}`
        );

        episodes.push({
          showId,
          episodeNumber: absoluteEpisode,
          isDownloaded: episodeStatusMap.has(absoluteEpisode)
            ? episodeStatusMap.get(absoluteEpisode)
            : false,
          magnetLink: null,
        });
      }
    ...
    console.log(
      `Generated ${episodes.length} episodes for the range S${startSeason}E${startEpisode} to S${endSeason}E${endEpisode}`
    );
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await db.delete(episodesTable).where(eq(episodesTable.showId, id));

    // Then delete the show
    const deletedShow = await db.delete(showsTable).where(eq(showsTable.id, id)).returning();
