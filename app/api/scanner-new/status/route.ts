...
const CONSECUTIVE_REQUEST_THRESHOLD = 10;
...
export async function GET() {
  try {
    ...
    if (now - lastRequestTime < CONSECUTIVE_REQUEST_WINDOW) {
      ...
      if (
        consecutiveRequests > CONSECUTIVE_REQUEST_THRESHOLD &&
        scanStateCache.data &&
    ...
    if (scanStateCache.data && now - scanStateCache.timestamp < effectiveTTL) {
      console.log(
        'DEBUG: Returning cached scan state, age:',
        now - scanStateCache.timestamp,
        'ms, isScanning:',
        scanStateCache.data.isScanning
      );
      return NextResponse.json(scanStateCache.data);
    ...
    try {
      ...
      if (
        !existingScanState ||
        existingScanState.length === 0 ||
        existingScanState[0].count === 0
      ) {
    ...
    if (scanStateChanged) {
      console.log(
        `Scan state changed: isScanning = ${response.isScanning}, status = ${response.status}`
      );
    ...
    console.log(
      'DEBUG: Returning fresh scan state, isScanning:',
      response.isScanning,
      'status:',
      response.status
    );
